...
function GitLabGlyph() {
  return (
    <svg viewBox="0 0 24 24" className="size-4" aria-hidden>
      <path
        fill="currentColor"
        d="M12 21.53 16.42 7.9h-8.84L12 21.53Z"
      />
      <path
        fill="currentColor"
        d="M12 21.53 7.58 7.9H3.9L12 21.53Z"
        opacity="0.72"
      />
      <path
        fill="currentColor"
      ...
      <path
        fill="currentColor"
        d="M12 21.53 16.42 7.9h3.68L12 21.53Z"
        opacity="0.72"
      />
      <path
        fill="currentColor"
        d="M7.58 7.9 9.38 2.37c.1-.3.51-.3.61 0L12 7.9H7.58Z"
      />
      <path
        fill="currentColor"
        d="M12 7.9 14 2.37c.1-.3.51-.3.61 0l1.81 5.53H12Z"
      />
    </svg>
  );
}
...
function getErrorMessage(error: unknown, gt: ReturnType<typeof useGT>) {
  ...
  return gt("We couldn’t sign you in. Check your credentials and try again.");
}
...
export function LoginForm() {
  ...
  useEffect(() => {
    ...
    router.replace("/");
    router.refresh();
  }, [isSessionPending, router, session?.session]);
      ...
      <form className="space-y-5" onSubmit={handleSubmit}>
        ...
        <Field
          ...
          hint={
            <Link
              href="#"
              className="text-[var(--accent-strong)] transition hover:text-[var(--accent)]"
            >
              {gt("Forgot?")}
            </Link>
          }
